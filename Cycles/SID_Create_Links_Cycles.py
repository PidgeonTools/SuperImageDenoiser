...
def create_links_cy(sid_denoiser_tree: NodeTree, settings: SID_Settings) -> NodeTree:
    ...
    if settings.use_transmission:
        ...
        sid_tree.links.new(input_node.outputs['Denoising Normal'],transmission_denoiser_node.inputs['Denoising Normal'])
        sid_tree.links.new(input_node.outputs['Denoising Albedo'],transmission_denoiser_node.inputs['Denoising Albedo'])
    ...
    if settings.use_emission: sid_tree.inputs.new("NodeSocketColor", "Emit")

    if settings.use_environment: sid_tree.inputs.new("NodeSocketColor", "Env")
    ...
    sid_tree.links.new(combine_node.outputs[0],output_node.inputs["Denoised Image"])
    sid_tree.links.new(seperate_node.outputs["R"],combine_node.inputs["R"])
    sid_tree.links.new(seperate_node.outputs["G"],combine_node.inputs["G"])
    sid_tree.links.new(seperate_node.outputs["B"],combine_node.inputs["B"])
    sid_tree.links.new(alpha_dn.outputs[0],combine_node.inputs["A"])
    sid_tree.links.new(final_dn.outputs[0],seperate_node.inputs[0])
